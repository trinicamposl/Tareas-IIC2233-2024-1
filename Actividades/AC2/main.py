...
class Jugador(Thread):
    ...
    def intentar_capturar_bandera(self) -> None: #revisar
    ...
    def intentar_robar_bandera(self) -> bool:
        ...
        if random() < self.PROBABILIDAD_ROBAR:
            self.lock_bandera.acquire(False)
            self.capturar_bandera()
    ...
    def correr_segunda_mitad(self) -> bool:
        self.lock_carrera.acquire(True)
        if self.senal_fin.is_set():
            self._correr = False
            return False
        
        if self.tiene_bandera:
            if self._posicion >= 100:
                self.senal_fin.set()
                self._correr = False
                return True
        else:
    ...
    def run(self) -> None:
        ...
        self.senal_inicio.wait() #revisar
